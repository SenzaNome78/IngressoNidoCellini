...
const long tempoTotaleAttesaBadgeXScrittura = 20000;
...
bool SendDataToWebServer(String userSerial, String ruolo)
{
	...
	String userS = userSerial;
	String userR = ruolo;
					   // *************** Usare GET ********************************************************
	httpC.begin(
		"http://192.168.0.2:80/NidoCellini/src/php/RegEntry.php?seriale="+ userSerial
		+ "&ruolo="
		+ ruolo);
...
bool AttivaModScrittura()
{
	...
	if (modScrittura)
	{
		...
		while (tempoAttesaBadgeXScrittura > 0)
		{
			...
			Serial.println("prima di client av");
			clientInterr = webServer.available();
			if (clientInterr.available())
			{
				interrompere = clientInterr.readStringUntil('\r');
				Serial.println(interrompere);
				delay(10);
				clientInterr.stop();
				//tempoAttesaBadgeXScrittura = 0;
			}

			Serial.println("dopo di client av");
		}
