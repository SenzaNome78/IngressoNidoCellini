...
void setup()
{
	...
	wServer.on("/RegEntry.html", HTTPMethod::HTTP_ANY, NewBadgeOnIn, NewBadgeOnOut);
...
void RegEntryOnIn()
{
	Serial.println("In RegEntryOnIn");
	...
	wServer.sendContent("RegEntryOnIn");

}

void RegEntryOnOut()
{
	Serial.println("In RegEntryOnOut");
}
...
int SendDataToWebServer(String userSerial, String ruolo, bool entrata)
{
	...
	String tmpStr = "http://192.168.0.2/NidoCellini/src/php/RegEntry.php";
		
	WiFiClient tmpClient = wServer.client();
	Serial.println(tmpClient.connect(new IPAddress(192,168,0,2), 80));

	//httpC.begin(tmpStr); // Apriamo una connessione http verso il server

	//httpCode = httpC.POST("seriale=" + userSerial + "&ruolo=" + ruolo + "&entrata=" + String(entrata));
